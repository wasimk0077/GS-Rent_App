...
class PropertyTypeBank extends StatefulWidget {
  const PropertyTypeBank({Key? key}) : super(key: key);

  @override
  State<PropertyTypeBank> createState() => _PropertyTypeBankState();
}

class _PropertyTypeBankState extends State<PropertyTypeBank> {
 int BankPropsTotal=0;
 int BankPropsVacant=0;
 int BankTotalRent=0;
 int BankTotalAsset=0;
  ...
  await FirebaseFirestore.instance.collection("property_main").get().then((value) {
    for(var i in value.docs) {
        ...
        if((i["Property_Details"]["Property_Types"]["Bank"])==true)
        {
          ...
          BankPropsTotal=BankPropsTotal+1;
          print(BankPropsTotal);
        }
 ...
 getData2() async {
  ...
  await FirebaseFirestore.instance.collection("property_main").get().then((value) {
    for(var i in value.docs) {
        ...
        if(((i["Property_Details"]["Property_Types"]["Bank"])==true)&&( i["Property_Details"]["vacant"]==true))
        {
...
getData3() async {
  ...
  await FirebaseFirestore.instance.collection("property_main").get().then((value) {
    for(var i in value.docs) {
         BankTotalAsset=BankTotalAsset+int.parse((i["Property_Details"]["Asset"]));
...
getData4() async {
  ...
  await FirebaseFirestore.instance.collection("property_main").get().then((value) {
    for(var i in value.docs) {
         BankTotalRent=BankTotalRent+int.parse((i["Property_Details"]["Rent"]));
    }
    // suggestionslist=Properties;
  });
}
  ...
  Widget build(BuildContext context) {
     return Scaffold(
                      ...
                      Container(
                       ...
                       Container(
                        ...
                        child: Row(
                          ...
                          Text(
                            (((BankPropsTotal-BankPropsVacant)/2).toString()),
                            style: TextStyle(
                              fontWeight:FontWeight.bold,
      ...
      child:Row(
        ...
        children: [
          Text(
                            ((((BankTotalAsset/2)*100)/(BankTotalRent/2)).toInt().toString()+"%"),
                            style: TextStyle(
                              fontWeight:FontWeight.bold,
